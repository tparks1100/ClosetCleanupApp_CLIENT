...
const SellClothes = ({ msgAlert, user, match }) => {
  ...
  let clothesToRender
  if (clothes) {
    clothesToRender = (clothes.filter(clothing => clothing.isWorn === false && clothing.status === 'Sell').map(clothing => {
      return (
...
export default withRouter(SellClothes)
